...
class ContextResponseAgent:
    ...
    def toolSelector(self, user_query: UserQuery) -> str:
        try:
            available_tools = [
                    ...
                    Tool(name="InternationalPlanChecker", description="Checks details of international plans. Plans that have international capabilities are only included in this tool."),
                Tool(name="Terms_condition_checker", description="Checks details of Terms Condition. If user has mentioned about return policy, warranty, bring your own tablet, or any policy related detailes then its best to retrive for relavent terms and services content"),
                Tool(name="Privacy_policy_checker", description="Checks details of Privacy Policy. If user has mentioned about privacy related details or queries then its best to retrive for relavent privacy content")
                ]
    ...
    def Terms_condition_checker(self, user_query: UserQuery) -> str:
        """
        Updated one Terms_condition_checker
        """
        try:
            with open("Terms_conditions_extracted.txt", "r") as file:
                context = file.read()
            ...
            return f"Error processing request: {str(e)}"
    def Privacy_policy_checker(self, user_query: UserQuery) -> str:
        """
        Updated one Privacy_policy_Extracted
        """
        try:
            with open("Privacy_policy_Extracted.txt", "r") as file:
                context = file.read()
